...
export default function AdminCommercePage() {
  ...
  function openConfirm(item: AdminOrder) {
    setConfirming(item);
    setReference("");
  }
  async function confirmOrder(event: React.FormEvent) {
    ...
    setSaving(true);
    try {
      toast.success(await confirmAdminOrder(confirming.id, reference.trim()));
      setConfirming(null);
      await load();
    } catch (err: any) {
  ...
  async function revoke(item: AdminCertificate) {
    if (!window.confirm(`Revoke certificate ${item.certificateNumber}?`))
      return;
    try {
      toast.success(await revokeAdminCertificate(item.id));
      await load();
    } catch (err: any) {
      toast.error(err?.message || "Unable to revoke certificate.");
    }
  }
  ...
  const filters =
    section === "orders" ? (
      <select
        ...
        value={orderStatus}
        onChange={(event) => {
          setOrderStatus(event.target.value as AdminOrder["status"] | "");
          setPage(1);
        }}
      >
      ...
      <form
        ...
        onSubmit={(event) => {
          event.preventDefault();
          setCourseId(courseInput ? Number(courseInput) : undefined);
          setPage(1);
        }}
      >
        <Input
  ...
  return (
    <>
      <PageHeading
        title="Commerce & learning"
        description={`${total} ${section}.`}
        action={filters}
      />
      <div className="mb-4 flex gap-2">
        {(["orders", "enrollments", "certificates"] as Section[]).map(
          (value) => (
            <Button
              key={value}
              size="sm"
          ...
          {section === "orders" ? (
                ...
                <table className="w-full min-w-[760px] text-left text-sm">
                  <thead className="border-b bg-slate-50 text-xs uppercase text-slate-500">
                    <tr>
                      ...
                      <th className="px-5 py-3">Course / plan</th>
                  ...
                  <tbody className="divide-y">
                    {orders.map((item) => (
                      <tr key={item.id}>
                        ...
                        <td className="px-5 py-4">
                          <span
                            className={`rounded-full px-2 py-1 text-xs ${item.status === "PAID" ? "bg-green-100 text-green-700" : item.status === "FAILED" ? "bg-red-100 text-red-700" : "bg-amber-100 text-amber-700"}`}
                          >
                            {item.status}
                          </span>
            ...
            !enrollments.length ? (
              <EmptyState
                ...
                description="No enrollments match this course."
              />
            ) : (
                ...
                >
                  <div>
                    ...
                    <p className="text-sm text-slate-500">
                      {item.userName} · {item.courseTitle}
                    </p>
        ...
        <DialogContent>
          <form onSubmit={confirmOrder}>
            <DialogHeader>
              ...
              <DialogDescription>
                Marks order {confirming?.id} paid and enrols{" "}
                {confirming?.courseTitle
                  ? `the student in ${confirming.courseTitle}`
                  : "the student"}
                . Use this only for payments collected outside the app.
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-2 py-5">
              ...
              <Input
                ...
                required
                placeholder="UPI transaction id, bank reference, receipt number"
                value={reference}
                onChange={(event) => setReference(event.target.value)}
              />
              <p className="text-xs text-slate-500">
                Amount due: {confirming?.currency} {confirming?.amount}
              </p>
            </div>
            <DialogFooter>
              <Button
