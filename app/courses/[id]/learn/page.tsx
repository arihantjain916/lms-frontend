...
export default function CourseLearnPage() {
  ...
  const loadCourse = useCallback(
    async (preferredLessonId?: string) => {
      setLoading(true);
      setError("");
      try {
        ...
        setLearning(result);
        const lesson =
          result.lessons.find((item) => item.id === preferredLessonId) ||
          result.lessons.find((item) => !item.isCompleted) ||
          result.lessons[0] ||
          null;
  ...
  useEffect(() => {
    ...
    lastSaved.current = currentLesson.watchedSeconds || 0;
    Promise.all([
  ...
  function selectLesson(lesson: LearningLesson) {
    ...
    if (currentLesson && currentSeconds !== undefined)
      updateProgress(currentLesson.id, currentSeconds).catch(() => undefined);
    setCurrentLesson(lesson);
    setPlayback(null);
    setResources([]);
  ...
  async function markComplete() {
    ...
    try {
      if (videoRef.current)
        await updateProgress(currentLesson.id, videoRef.current.currentTime);
      ...
      toast.error(requestError?.message || "Unable to complete lesson");
    } finally {
      setCompleting(false);
    }
  }
  ...
  async function download(resource: LessonResource) {
    try {
      ...
      toast.error(requestError?.message || "Unable to download resource");
    }
  }
  ...
  const currentIndex = currentLesson
    ? learning.lessons.findIndex((item) => item.id === currentLesson.id)
    : -1;
  const nextLesson =
    currentIndex >= 0 ? learning.lessons[currentIndex + 1] : undefined;

  return (
    <main className="min-h-screen bg-slate-950 text-white">
      <header className="border-b border-white/10 bg-blue-950 px-4 py-3">
          ...
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-blue-200">
                {learning.progress.completedLessons} of{" "}
                {learning.progress.totalLessons} lessons completed
              </p>
            </div>
          </div>
          <div className="hidden w-64 items-center gap-3 sm:flex">
            <Progress
              value={learning.progress.progressPercent}
              className="h-2 bg-white/20"
            />
            <span className="text-sm font-semibold">
              {Math.round(learning.progress.progressPercent)}%
            </span>
          </div>
          {learning.progress.isCompleted && (
            <Button
              size="sm"
      ...
      <div className="border-b border-white/10 bg-slate-900 px-4 py-2 text-right">
        ...
        >
          <Link href={`/courses/${id}/exams`}>Course exams</Link>
        </Button>
      </div>
      <div className="mx-auto grid max-w-[1600px] lg:grid-cols-[340px_1fr]">
        <aside className="max-h-[calc(100vh-65px)] overflow-y-auto border-r border-white/10 bg-slate-900">
          <div className="sticky top-0 border-b border-white/10 bg-slate-900 p-4">
          ...
          <div>
            {learning.lessons.map((lesson, index) => (
              <button
                key={lesson.id}
                className={`flex w-full gap-3 border-b border-white/5 p-4 text-left transition hover:bg-white/5 ${lesson.id === currentLesson?.id ? "bg-blue-600/20" : ""}`}
              ...
              >
                <div className="mt-0.5 shrink-0">
                  {lesson.isCompleted ? (
                    ...
                    <span className="flex h-5 w-5 items-center justify-center rounded-full border border-slate-600 text-[10px]">
                      {index + 1}
                    </span>
                  )}
                </div>
                <div className="min-w-0">
                  ...
                  <div className="mt-1 flex gap-2 text-xs text-slate-400">
                    ...
                    {lesson.watchedSeconds > 0 && !lesson.isCompleted && (
                      <span>
                        • Resume at {Math.floor(lesson.watchedSeconds / 60)}m
                      </span>
                    )}
                  </div>
                </div>
        ...
        <section className="min-w-0 bg-slate-950">
          <div className="aspect-video w-full bg-black">
            {lessonLoading ? (
              ...
              <video
                key={playback.lessonId}
                ref={videoRef}
                src={playback.videoUrl}
                controls
                className="h-full w-full"
                onLoadedMetadata={() => {
                  if (videoRef.current && playback.watchedSeconds > 0)
                    videoRef.current.currentTime = playback.watchedSeconds;
                }}
                onTimeUpdate={saveWatchedTime}
                onPause={() => {
              ...
              <div className="flex h-full flex-col items-center justify-center text-slate-400">
                ...
                <p className="mt-3">
                  No playback video available for this lesson.
                </p>
              </div>
            )}
          </div>
                ...
                <div className="mb-2 flex gap-2">
                  {currentLesson?.status && (
                    <Badge variant="secondary" className="capitalize">
                      {currentLesson.status.toLowerCase()}
                    </Badge>
                  )}
