...
export default function InstructorLayout({
  ...
  if (!["INSTRUCTOR", "ADMIN"].includes(user.role?.toUpperCase()))
    return (
      <main className="grid min-h-screen place-items-center bg-slate-50 p-6">
        <div className="max-w-md rounded-2xl border bg-white p-8 text-center shadow-sm">
          <ShieldAlert className="mx-auto mb-4 h-10 w-10 text-amber-500" />
          <h1 className="text-xl font-semibold">Instructor access required</h1>
          <p className="mt-2 text-sm text-slate-500">
            Your account cannot open the teaching workspace.
          </p>
    ...
    <div className="flex h-full flex-col bg-slate-950 text-slate-200">
      <div className="flex h-20 items-center gap-3 border-b border-white/10 px-6">
        ...
        <div>
          ...
          <p className="text-xs text-slate-400">Instructor workspace</p>
        </div>
      </div>
