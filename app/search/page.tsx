...
export default function SearchPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(e?.message || "Unable to load search results");
    } finally {
      setLoading(false);
    }
  ...
  const total = Object.values(results.totals).reduce(
    (sum, count) => sum + count,
    0,
  );
    ...
    <main className="container py-8">
      ...
      <form onSubmit={submit} className="relative mb-7">
        ...
        <Input
          ...
          className="py-6 pl-10 pr-28 text-lg"
          placeholder="Search courses, webinars and articles..."
        />
      ...
      <div className="grid gap-8 lg:grid-cols-[240px_1fr]">
        <aside
          className={`${showFilters ? "block" : "hidden"} space-y-6 rounded-lg border p-4 lg:block`}
        >
          <div>
            ...
            {facets.categories.map((facet) => (
              ...
              >
                <Checkbox
                  checked={categories.includes(facet.id)}
                  onCheckedChange={() =>
                    toggle(facet.id, categories, setCategories)
                  }
                />
          ...
          <div>
            <h2 className="mb-3 font-semibold">Course level</h2>
            {facets.levels.map((facet) => (
              ...
              >
                <Checkbox
                  ...
                  onCheckedChange={() => toggle(facet.id, levels, setLevels)}
                />
...
function ResultCard({ item }: { item: SearchItem }) {
  return (
    <Link
      href={`${paths[item.type]}/${item.slug || item.id}`}
    ...
    >
      ...
      <div className="p-5">
        ...
        <h2 className="line-clamp-2 text-lg font-semibold group-hover:text-blue-700">
          {item.title}
        </h2>
